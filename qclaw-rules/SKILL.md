...
以 `__openclaw_memory_core_` 开头的消息（如 `__openclaw_memory_core_short_term_promotion_dream__`、`__openclaw_memory_core_light_sleep__`、`__openclaw_memory_core_rem_sleep__`）是 **OpenClaw 记忆系统的内部心跳消息**，由系统自动触发，不是用户发送的。

**必须遵守以下规则**：

1. **不要删除** `Memory Dreaming Promotion` 定时任务 — 它是记忆整理系统的核心调度器，删除后记忆功能将停止工作
2. **不要将这些消息记录到 daily memory** — 它们是技术性心跳，不包含有意义的用户信息
3. **不要向用户报告这些消息** — 用户不需要知道内部心跳的存在
4. **简单忽略即可** — 收到这类消息时，不需要做任何操作，静默处理
