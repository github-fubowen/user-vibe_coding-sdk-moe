...
description: 腾讯问卷（wj.qq.com）-
  ...
  获取问卷详情（标题、设置、页面、题目、选项完整结构 + 纯文本 DSL）(2) 使用纯文本创建问卷（text 必填，支持指定场景/指定项目）(3)
  更新问卷中的单个题目（DSL 格式）(4) 获取问卷回答列表（支持游标分页）。支持场景：调查(1)、考试(3)、测评(6)、投票(8)。
...
根据当前操作系统选择对应的脚本：
- **macOS / Linux**：使用 `setup.sh`（Bash）
- **Windows**：使用 `setup.ps1`（PowerShell）
...
**Windows (PowerShell)：**
```powershell
$env:TENCENT_SURVEY_TOKEN="xxx"; powershell -ExecutionPolicy Bypass -File "${SKILL_DIR}\setup.ps1" wj_check_and_start_auth
```
...
**Windows (PowerShell)：**
1. 执行 `powershell -ExecutionPolicy Bypass -File "${SKILL_DIR}\setup.ps1" wj_check_and_start_auth`
2. 输出 `READY` → 鉴权已就绪，直接继续
3. 输出 `AUTH_REQUIRED:<url>` → 向用户展示授权链接，然后执行 `powershell -ExecutionPolicy Bypass -File "${SKILL_DIR}\setup.ps1" wj_wait_auth` 等待授权完成
4. 输出 `ERROR:*` → 告知用户对应错误
...
### mcporter CLI 调用格式（跨平台）

**macOS / Linux (bash)：**
```bash
mcporter call tencent-survey.create_survey --args '{"text": "问卷标题\n\n1. 题目[单选题]\n选项A\n选项B"}'
```

**Windows (PowerShell) — 推荐使用位置参数方式：**
```powershell
mcporter call tencent-survey.create_survey --text "问卷标题\n\n1. 题目[单选题]\n选项A\n选项B"
```

> ⚠️ **Windows 注意**：PowerShell 下 `--args` 的 JSON 参数容易被错误解析。推荐使用位置参数方式（`--text "..."` / `--survey_id 12345`），或使用如下转义：
> ```powershell
> mcporter call tencent-survey.create_survey --args '{\"text\": \"内容\"}'
> ```
