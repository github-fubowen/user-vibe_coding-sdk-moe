...
Usage:
  ...
  python privacy-scan.py --include-runtime       # also scan .workbuddy host dirs
...
# v2.7.1 (F-15): WorkBuddy automations write `.workbuddy/{automations,memory}/`
# into their cwd. When cwd IS the SDK dir these host-runtime files pollute the
# scan domain (automation memory quotes `D:\...` commands by design) and turned
# ci-smoke permanently red. Skip these two — but only as DIRECT children of
# `.workbuddy/`; `.workbuddy/debug-cases/` is a shipped deliverable and stays
# scanned. `--include-runtime` disables the skip (audit the host files too).
RUNTIME_SKIP_DIR_NAMES = {"automations", "memory"}
...
def main() -> int:
    ...
    ap.add_argument("--include-runtime", action="store_true",
                    help="scan .workbuddy/{automations,memory} host-runtime dirs too "
                         "(default: skipped — F-15, they quote machine paths by design)")
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        if (not args.include_runtime and rel_parts
                and rel_parts[-1] == ".workbuddy"):
            before = len(dirnames)
            dirnames[:] = [d for d in dirnames if d not in RUNTIME_SKIP_DIR_NAMES]
            skipped += before - len(dirnames)
        ...
        for fn in filenames:
            ...
            if fp.is_symlink():
                skipped += 1
                continue
            if fp.name in ("privacy-scan.py", "robustness-suite.py"):
                skipped += 1   # self-skip: scanner regex literals + privacy test
                continue       # fixtures quote machine paths BY DESIGN (v2.7.1)
            scanned += 1
