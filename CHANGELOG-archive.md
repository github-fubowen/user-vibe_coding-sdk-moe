# CHANGELOG-archive — user-vibe_coding-sdk-moe

> 归档策略（F-62）：CHANGELOG.md 条目 >30 天移入本文件；主文件只保留近期条目 + 指针行。
> v1.x 时代（v1.0 ~ v1.16.0，2026-08-15 ~ 08-22，v2.0 重写前历史）于 2026-09-04 预归档。
> 本文件为冷历史，不参与 version-check 顺序闸（其只校验 CHANGELOG.md 顶部条目）。
