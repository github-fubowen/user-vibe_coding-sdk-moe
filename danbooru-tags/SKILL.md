...
当指定 group 查询返回空或无 confirmed 时，执行以下回退：

1. **原 group 查不到 → 用 `--group general` 重试**：部分标签（如 `garter_belt`）在 clothing 组不存在，但在 general 组存在 exact_tag
2. **general 仍查不到 → nltags**：极少数场景标签（如 `helipad`）在所有组均不存在，正确降级为自然语言描述
3. **回退命中后处理**：在 P3 阶段手动将 general 组命中的标签归入原始语义类别（clothing/scene/pose 等）

示例：
```
# clothing 组空 → 回退 general
cli --group clothing --keyword "garter belt"     # → 空
cli --group general --keyword "garter belt"      # → ✅ exact_tag
# P3 时手动归类: garter_belt → clothing
```
