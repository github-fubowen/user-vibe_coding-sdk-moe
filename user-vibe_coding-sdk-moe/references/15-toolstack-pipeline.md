...
> **v2.7.1（F-24）**：非交互拒绝为双保险——`SDK_NONINTERACTIVE=1` 环境变量（套件/自动化注入的确定性口）或 stdin 非 tty；上游 `gh api` 单次 15s + 上游阶段 30s 总预算（逐调用强制），超时/超预算降级为 note 不炸跑。背景：部分宿主（ConPTY/沙箱 shim）下 `isatty()` 对 NUL stdin 也可能返回 True，仅靠 tty 检测不可靠。
