...
| `patch-gate.py`（v2.10.0） | 补丁规模预算闸 | numstat/diff/stat 输入；files≤5 / lines≤300 / 依赖≤1（kernel §I）；超限 exit 2 → ESCALATED；diff-risk `--verify-confidence` 合成 `repair_confidence`<0.7 → 人工（T-23 / F-29） |
