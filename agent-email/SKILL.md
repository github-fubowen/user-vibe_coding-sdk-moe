---
name: agent-email
description: 通过 QQ Agent 邮箱（OAuth 2.1）专用 CLI 工具 `agently-cli`
  操作邮件：发送、回复、转发、搜索、读取、下载附件、管理收件箱。Agent 邮箱是 QClaw 为用户生成的专属 AI 邮箱（形如
  qclawXXXX@agent.qq.com），用户可将该邮箱分享给他人，由 QClaw 自动接收并处理来信。CLI
  安装和鉴权后台静默完成，对用户完全透明，凭证不暴露给用户。
metadata:
  openclaw:
    emoji: 🤖
    requires:
