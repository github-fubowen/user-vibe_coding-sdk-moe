...
description: 个人邮箱二级路由层。仅在 email-skill 把任务转发到本 skill 后被调用——典型场景是『发给别人 / 需要附件 / 抄送
  / 收件箱搜索 / 下载附件』等完整收发需求。本 skill 通过执行路由脚本确定性地决定路由到 agent-email 或
  imap-smtp-email，然后立即用 read 工具读取目标 SKILL.md 执行邮件操作。
trigger_keywords:
  - 给客户发邮件
  - 给同事发邮件
  - 抄送
